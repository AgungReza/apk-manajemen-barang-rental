            ...
            <form action="/transaksi/save" method="post">
                ...
                <div class="mb-4 grid grid-cols-2 gap-4">
                    <div>
                        ...
                        <input id="tanggal_pinjam" name="tanggal_pinjam" type="date" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm" value="<?= date('Y-m-d') ?>" />
                    </div>
                    <div>
                        <label for="tanggal_kembali" class="block text-sm font-medium text-gray-700">Tanggal Kembali</label>
                        <input id="tanggal_kembali" name="tanggal_kembali" type="date" class="mt-1 block w-full rounded-md border-gray-300 shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm" />
                    </div>
                </div>
            ...
            if (data.length > 0) {
                data.forEach(customer => {
                    ...
                    item.addEventListener('click', () => {
                        ...
                        document.getElementById('customer_id').value = customer.customer_id; // Set customer_id
                        customerSuggestions.classList.add('hidden');
                        noCustomerFound.classList.add('hidden');
                    });
